...
// ── 1. Variant Definition (Fluid Tech Palette) ──
const variantClasses: Record<Variant, string> = {
  // Primary: الأحمر الملكي بظلال متوهجة ناعمة
  primary: 'bg-[#c10100] text-white border-transparent hover:bg-[#a30100] hover:shadow-xl hover:shadow-[#c10100]/20 shadow-sm',
  // Accent: الأسود الصناعي الفخم
  accent:  'bg-[#131414] text-white border-transparent hover:bg-black hover:shadow-xl hover:shadow-black/10 shadow-sm',
  // Outline: إطار تقني رفيع بلمسة زجاجية عند التمرير
  outline: 'bg-transparent text-[#131414] border-gray-200 hover:border-[#c10100] hover:text-[#c10100] hover:bg-white hover:shadow-lg hover:shadow-black/[0.02]',
  // Ghost: للتنقل الهادئ
...
const BASE_CLASSES = `
  ...
  focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#c10100]/20
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'md', loading, className, children, disabled, ...props }, ref) => (
    <button
      ...
      {...props}
    >
      {/* ── Visual Ripple Effect Background (Decorative) ── */}
