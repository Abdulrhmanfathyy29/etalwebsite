    ...
    <section className="section-pad bg-[#fcfcfc] overflow-hidden">
      <div className="container-etal space-y-12">
        ...
        <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between px-2">
          ...
          />
          <Link href="/products" className="group flex items-center gap-4 bg-white border border-gray-100 pl-6 pr-2 py-2 rounded-full shadow-sm hover:shadow-md hover:border-[#229264]/30 transition-all duration-300">
            <span className="text-[12px] font-bold uppercase tracking-widest text-[#131414]">All Products</span>
...
function DynamicCard({ product }: { product: ProductWithCategory }) {
  return (
    <Link href={`/products/${product.slug}`} className={CARD_CLASSES}>
      {/* Container for Image with Pill-like rounding */}
      <div className="relative aspect-square overflow-hidden rounded-[2rem] bg-[#f8f9fa] flex items-center justify-center p-8 transition-colors duration-500 group-hover:bg-[#229264]/5">
        ...
        {/* Floating ID Label */}
        <div className="absolute top-4 right-4 h-8 w-8 rounded-full bg-white/80 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-sm">
      ...
      <div className="flex flex-1 flex-col p-6 pb-8">
        ...
        <div className="mt-8 flex items-center justify-between">
           ...
           <div className="h-8 w-8 rounded-full bg-gray-50 flex items-center justify-center">
             <ArrowRight size={14} className="text-[#229264]" />
           </div>
        </div>
      </div>
    </Link>
